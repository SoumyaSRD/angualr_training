...
export class ChatbotComponent {
    ...
    private knowledgeBase = this.buildKnowledgeBase();
    ...
    private generateResponse(query: string): ChatMessage {
        ...
        for (const [key, values] of Object.entries(this.knowledgeBase)) {
        ...
        for (const topic of this.navigationService.topics) {
            if (lowerQuery.includes(topic.title.toLowerCase())) {
                const subTopics = topic.subTopics.slice(0, 3).map(s => s.title);
                return {
                    type: 'bot',
